...
layout (location = 10) uniform vec4 materialAmbient;
layout (location = 11) uniform vec4 materialDiffuse;
layout (location = 12) uniform vec4 materialSpecular;
layout (location = 13) uniform float materialShininess;




layout (location = 16) uniform float lightflickering;
...
void main() {
    ...
    flickering=  fs_in.lightIntensity;//g(pow(sin(lightflickering+20*i),2)*0.4f+pow(cos(lightflickering*3+20*i),2)*0.5f+pow(sin(8*lightflickering+20*i),2)*0.3f+pow(cos(5*lightflickering+20*i),2)*pow(sin(lightflickering*0.7+20*i),2)+1);
    attenuation =1/(1+0.02*length(fs_in.L)+0.1*pow(length(fs_in.L)/5,2));
    L = normalize(fs_in.L);
    H = normalize(L + V);
    ...
    FragColor = vec4(ambient +endDif+endSpec , 1.0);
}